...
public interface OrderRepo extends JpaRepository<Order, UUID> {
    ...
    @Query(value = "SELECT BIN_TO_UUID(o.id) AS orderId, o.status, p.name AS product, " +
            "BIN_TO_UUID(p.id) AS productId, po.unit_price, po.created_at, " +
            "po.quantity, po.total_price, o.total_price AS totals, BIN_TO_UUID(u.id) AS userId, " +
            "CONCAT(u.first_name, ' ', u.last_name) AS customer, u.username, u.email FROM product_order_tb po " +
...
//            "JOIN vendor_tb v ON v.user_id=p.product_owner_id " +
    ...
    @Query(value = "SELECT BIN_TO_UUID(o.id) AS orderId, " +
            "o.status, p.name AS product, BIN_TO_UUID(p.id) AS productId, " +
            "v.name AS vendor, po.unit_price, po.created_at, po.quantity, po.total_price, " +
            "o.total_price AS totals, BIN_TO_UUID(u.id) AS userId, CONCAT(u.first_name, ' ', u.last_name) AS customer," +
            " u.username, u.email FROM product_order_tb po " +
            "JOIN product p ON po.product_id=p.id " +
            "JOIN order_tb o ON o.id=po.order_id " +
            "JOIN vendor_tb v ON v.user_id=p.product_owner_id " +
    ...
    @Query(value = "SELECT BIN_TO_UUID(o.id) AS orderId, o.status, p.name AS product, " +
            "BIN_TO_UUID(p.id) AS productId, v.name AS vendor, po.unit_price, po.created_at," +
            " po.quantity, po.total_price, o.total_price AS totals, BIN_TO_UUID(u.id) AS userId, " +
            "CONCAT(u.first_name, ' ', u.last_name) AS customer, u.username, u.email FROM product_order_tb po " +
            "JOIN product p ON po.product_id=p.id " +
            "JOIN order_tb o ON o.id=po.order_id " +
